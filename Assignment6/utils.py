...
def draw_robot(x, b, color="gray", alpha=1, label="", ax=None):
    ...
    wheel_r = patches.Rectangle(
        [-wheel_diameter / 2.0, +b / 2 - wheel_width],
        wheel_diameter,
        wheel_width,
        color="black",
        alpha=alpha,
    ...
    for element in elements:
        element.set_transform(
            mpl.transforms.Affine2D().rotate(x[2])
            + mpl.transforms.Affine2D().translate(x[0], x[1])
            + ax.transData
        )
    ...
    ax.annotate(
        "",
        xy=(x[0] + b / 1.9 * np.cos(x[2]), x[1] + b / 1.9 * np.sin(x[2])),
        xytext=(x[0] + b / 5 * np.cos(x[2]), x[1] + b / 5 * np.sin(x[2])),
...
def draw_state_distribution(x, P, alpha=0.9, ax=None):
    ...
    z = var.pdf(X).reshape(1000, 1000)
    z[z < 1e-5] = 1e-5  # BUGFIX for matplotlib > 3
...
def draw_map(M, ax=None, color="red", dashed=False, linewidth=1, alpha=1):
    ...
    for alp, rho in M:
        ...
        ax.plot(
            ...
            [y1, y2],
            color=color,
            ls="dashed" if dashed else "-",
            linewidth=linewidth,
            alpha=alpha,
        )


def draw_map_estimate(x, P, n_samples=10, ax=None, color="red", linewidth=1, alpha=1):
    if ax is None:
        fig = plt.gcf()
        ax = fig.gca()
    ...
    draw_map(samples.reshape(-1, 2).T, alpha=0.1, dashed=False, ax=ax, color=color)
    draw_map(x.reshape(-1, 2).T, alpha=1, dashed=False, ax=ax, color=color)
...
def draw_landmarks_distribution(M, P_M, ax=None):
    if ax is None:
        fig = plt.gcf()
        ax = fig.gca()

    ax.set_xlabel("$\\alpha$", size=15)
    ax.set_ylabel("$\\rho$", size=15)

    if not isinstance(M, list):
        M = np.split(M, len(M) // 2)
    if not isinstance(P_M, list):
        idx = 0
        P_M_ = []
        while idx < P_M.shape[0]:
            P_M_.append(P_M[idx : idx + 2, idx : idx + 2])
            idx += 2
        P_M = P_M_

    for i, m in enumerate(M):
        draw_state_distribution(m, P_M[i], ax=ax)


def draw_landmarks(M, color="red", ax=None):
    if ax is None:
        fig = plt.gcf()
        ax = fig.gca()

    for i, m in enumerate(M):
        ax.plot(m[0], m[1], "o", color=color)
        ax.text(m[0], m[1], "${\\bf m}^%d$" % (i + 1), size=15)
...
def generate_instructions(n):
    return [
        np.random.multivariate_normal(np.ones(2) * 0.5, np.eye(2) * 0.1)
        for _ in range(n)
    ]
...
def get_default_map():
    return [[np.pi / 4.0, -3], [np.pi / 2.0, -3], [0, 2], [np.pi / 2.0, 3], [0, -3]]


def get_sampled_map(k1=0.1, k2=1, n=7):
    M = []
    P = []
    for i, m in enumerate(get_default_map()):
        alpha, rho = m[0], m[1]
        alpha_samples = np.random.normal(alpha, k1 * (0.05 if i < 2 else 1), n)
        rho_samples = np.random.normal(rho, k2 * (0.05 if i < 2 else 1), n)
        M.append(np.array([alpha_samples.mean(), rho_samples.mean()]))

        cov = np.eye(2)
        cov[0, 0] = k1 * (0.05 if i < 2 else 1)
        cov[1, 1] = k2 * (0.05 if i < 2 else 1)
        P.append(cov)
